...
class CreativeWork extends SchemaOrg
{
    ...
    public static function fromTei($tei)
    {
        $service = new \Sabre\Xml\Service();
        $service->elementMap = [
            '{}author' => 'Sabre\Xml\Deserializer\keyValue',
            '{}editor' => 'Sabre\Xml\Deserializer\keyValue',
            '{}respStatement' => 'Sabre\Xml\Deserializer\keyValue',
            '{}persName' => 'Sabre\Xml\Deserializer\keyValue',
            '{}series' => 'Sabre\Xml\Deserializer\keyValue',
            '{}imprint' => 'Sabre\Xml\Deserializer\keyValue',
        ];

        $listBibl = $service->parse('<listBibl>' . $tei . '</listBibl>');
        $biblStruct = $listBibl[0];

        $data = [
            'key' => $biblStruct['attributes']['id'],
            'version' => $biblStruct['attributes']['n'],
            'itemType' => $biblStruct['attributes']['type'],
            'slug' => $biblStruct['attributes']['corresp'],
            'creators' => [],
        ];

        foreach ($biblStruct['value'] as $elem) {
            switch ($elem['name']) {
                case '{}analytic':
                case '{}monogr':
                    foreach ($elem['value'] as $subelem) {
                        switch ($subelem['name']) {
                            case '{}title':
                                ...
                                $data[$key] = $subelem['value'];
                                break;

                            case '{}idno':
                                switch ($subelem['attributes']['type']) {
                                    case 'ISBN':
                                        $data['ISBN'] = $subelem['value'];
                                        break;
                                }
                                break;
                            ...
                            case '{}respStatement':
                                ...
                                if (array_key_exists('{}forename', $value)) {
                                    $creator['firstName'] = $value['{}forename'];
                                    if (array_key_exists('{}surname', $value)) {
                                        $creator['lastName'] = $value['{}surname'];
                                    }
                                }
                                else {
    ...
    public function teiSerialize()
    {
        $biblStruct = new FluidXml(null);
        $biblStruct->namespace('tei', 'http://www.tei-c.org/ns/1.0', FluidNamespace::MODE_IMPLICIT);

        // the following two are the other way around compared to Zotero-TEI Export because
        // only the key is guaranteed to be unique, while slugs can collide for multiple pubs in the same year
        $attrs = [];
        if (!empty($this->id)) {
            $attrs['id'] = $this->id; // we use id and not xml:id due to: attribute value 3E5E4TY9 is not an NCName
        }

        if (!empty($this->itemType)) {
            $attrs['type'] = $this->itemType;
        }

        if (!empty($this->slug)) {
            $attrs['corresp'] = $this->slug;
        }

        // for sync
        if (!empty($this->version)) {
            $attrs['n'] = $this->version;
        }

        $root = $biblStruct->add('tei:biblStruct', true, $attrs);
        ...
        foreach ($this->creators as $creator) {
            if (!in_array($creator['creatorType'], ['author', 'editor'])) {
                ...
                $respStatement->addChild('resp', self::xmlSpecialchars($creator['creatorType']));
                $entry = $respStatement->addChild('persName', true);
            }
            else {
                $entry = !is_null($analytic) && 'author' == $creator['creatorType']
                    ? $analytic->addChild($creator['creatorType'], true)
            ...
            foreach (['firstName' => 'forename', 'lastName' => 'surname', 'name' => 'name'] as $src => $target) {
                if (!empty($creator[$src])) {
                    ...
                    if ('surname' == $tag && empty($creator['firstName'])) {
                        // use name instead of surname if there is no forename
                        $tag = 'name';
                    }

                    $entry->addChild($tag, self::xmlSpecialchars($creator[$src]));
                }
            }
        }

        if (!empty($this->bookEdition)) {
            $monogr->addChild('edition', self::xmlSpecialchars($this->bookEdition));
        }
        ...
        foreach ([
            ...
            if (!empty($this->$src)) {
                ...
                $attrs = [];
                if (is_array($target)) {
                    $tags = array_keys($target);
                    $tag = $tags[0];
                    $attrs = $target[$tag];
                }

                $imprint->addChild($tag, self::xmlSpecialchars($this->$src), $attrs);
            }
        }
