...
--></xsl:template>
  ...
  <!-- override version from dta-base.xsl to add support for @class -->
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <p>
          ...
          <xsl:call-template name="applyRendition">
            <xsl:with-param name="class"><xsl:value-of select="$class" /><xsl:if test="@class"> dta-p-<xsl:value-of select="@class"></xsl:value-of></xsl:if></xsl:with-param>
          </xsl:call-template>
          <xsl:apply-templates/>
        </p>
      </xsl:otherwise>
